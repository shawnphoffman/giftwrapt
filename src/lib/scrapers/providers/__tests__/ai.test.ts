...
beforeEach(() => {
	mockAiValid = false
	fetchQueue = []
	// `safeFetch` resolves the URL's hostname before fetching. The test
	// URLs don't resolve, so stub the resolver to return a public IP.
	// SSRF behavior is exercised in `safe-fetch.test.ts`.
