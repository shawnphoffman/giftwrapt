...
// validates.
//
// FLAT ROUTE (`sign-in_.mobile-passkey.tsx`). The trailing `_` opts
// this page out of nesting under `sign-in.tsx`; without it, TanStack
// Router treats this file as a child of the regular sign-in route,
// which has no `<Outlet />`, so the page silently rendered the web
// sign-in form instead of the passkey-only stub. The URL path stays
// `/sign-in/mobile-passkey`; only the file naming changes.
...
export const Route = createFileRoute('/(auth)/sign-in_/mobile-passkey')({
...
function MobilePasskeyPage() {
	...
	const handlePasskey = async () => {
		...
		try {
			...
			// Full-page replace into the mobile-api callback so the
			// freshly-set session cookie rides along. `_native-done`
			// mints the apiKey + 302s to the iOS custom scheme, which
			// `ASWebAuthenticationSession` captures. `replace` keeps
			// this page out of history.
