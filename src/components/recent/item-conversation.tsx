...
export default function ItemConversation(props: ItemConversationProps) {
	const {
		id,
		title,
		url,
	...
	return (
		<div className="relative">
			...
			<div
				className={cn(
					'relative z-10 flex flex-col ring-1 ring-inset ring-border rounded-lg bg-card shadow-sm overflow-hidden',
					priorityRingClass[priority]
				)}
			>
				{/* ITEM HEADER */}
				<div className="flex items-stretch gap-3 p-3 ps-4">
					...
					<div className="border-t bg-muted/30 flex flex-col">
						{hiddenComments > 0 && (
							<Link
								...
								params={{ listId: String(listId) }}
								hash={`item-${id}`}
								className="flex items-center gap-1.5 px-4 py-2 text-xs text-muted-foreground hover:text-foreground hover:bg-muted/50 cursor-pointer border-b"
							>
								<MessageSquare className="size-3" />
								<span>
									{hiddenComments} earlier comment{hiddenComments !== 1 ? 's' : ''}
								</span>
							</Link>
						)}
						<div className="flex flex-col divide-y">
							{[...comments].reverse().map(c => {
								const commenterName = c.user.name || c.user.email
								return (
									<div key={c.id} className="flex gap-3 px-4 py-3">
										<UserAvatar name={commenterName} image={c.user.image} size="small" className="shrink-0" />
