...
export function ItemImage({ src, alt, className }: Props) {
	...
	return (
		<>
			...
			>
				<img src={src} alt={alt} className="object-contain w-16 max-h-16 xs:w-24 xs:max-h-24 transition-transform group-hover:scale-105" />
			</button>

			<Dialog open={open} onOpenChange={setOpen}>
				<DialogContent className="max-w-[min(90vw,48rem)] sm:max-w-[min(90vw,48rem)] p-2 bg-popover" showCloseButton>
					<DialogTitle className="sr-only">{alt}</DialogTitle>
					<img src={src} alt={alt} className="w-full h-auto max-h-[80vh] object-contain rounded-md" />
				</DialogContent>
			</Dialog>
		</>
	)
