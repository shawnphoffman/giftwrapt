		...
		<>
			{messages.map((msg: Message) => (
				<div key={msg.id} className={`flex ${msg.user === user ? 'justify-end' : 'justify-start'}`}>
					<div className={`flex items-start space-x-3 max-w-xs lg:max-w-md ${msg.user === user ? 'flex-row-reverse space-x-reverse' : ''}`}>
						<div
						...
						<div
							className={`px-4 py-2 rounded-2xl ${
								msg.user === user ? 'bg-blue-500 text-white rounded-br-md' : 'bg-white text-gray-800 border border-gray-200 rounded-bl-md'
							}`}
						>
							{msg.user !== user && <p className="text-xs text-gray-500 mb-1 font-medium">{msg.user}</p>}
